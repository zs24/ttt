...
//       
   ...
   private void setDefaults () {
      ...
      if ( this.get("user/systime")            == null ) { this.put("user/systime","00:00:00"); }
      if ( this.get("user/injtime")            == null ) { this.put("user/injtime","00:00:00"); }
      if ( this.get("user/clocktime")          == null ) { this.put("user/clocktime","00:00:00"); }
...
//      if ( this.get("user/max_vol_mass")   == null ) { this.put("user/max_vol_mass",0.0); }
   ...
   public void readXMLin () throws PumpException {
      ...
      this.put("user/clocktime"       , xml.getNestedTagValue(this.xmlroot+"user/clocktime")       , Types.STRING);            
...
// 
   ...
   public void calc_predictedDose () {
      ...
      Double    dose0   =    (Double)this.get("user/dose0");
      Timestamp time0   = (Timestamp)this.get("user/time0",Types.TIME);
      Timestamp start   = (Timestamp)this.get("user/clocktime",Types.TIME);
      Double    hl      =    (Double)this.get("config/hl");
      Double dose_predicted;
   
      if ( frac == null ) { return; }
   
      dose_inj = Decay.activity(dose0,time0,start,hl,Decay.HalfLifeUnits.Minutes); 
...
//       System.out.println("dose_predicted: "+dose_predicted);
   ...
   public void calc_actualDose () {       
      ...
      preinj_dose  = Decay.activity(dose0,time0,start,hl,Decay.HalfLifeUnits.Minutes); 
      postinj_dose = Decay.activity(res_dose,res_time,start,hl,Decay.HalfLifeUnits.Minutes); 
...
// 
   ...
   public String report () {
      ...
      double    vtot      =    (Double)this.get("user/vtot");
      double    hl        =    (Double)this.get("config/hl");
      Timestamp systime   = (Timestamp)this.get("user/systime",Types.TIME);
      Timestamp injtime   = (Timestamp)this.get("user/injtime",Types.TIME);
      Timestamp clocktime = (Timestamp)this.get("user/clocktime",Types.TIME);
      Timestamp res_time  = (Timestamp)this.get("user/res_time",Types.TIME);
      double    res_dose  =    (Double)this.get("user/res_dose");
      ...
      report += String.format("%19s  \n","");
      report += String.format("%19s  \n",clocktime.toString().substring(0,19));
      report += "\n";
      report += "\n";
      report += "\n";
      ...
      if ( ! this.getString("user/injtime").equals("00:00:00") ) {
         report += String.format("%19s  %20s\n",  systime.toString().substring(0,19),"infuse button pressed");
         report += String.format("%19s  %20s\n",  injtime.toString().substring(0,19),"pump started turning");
         report += String.format("%19s  %20s\n",clocktime.toString().substring(0,19),"injection clock started");
      }
      report += "-\n";  // paper print-out seems to drop last line.  make sure its one we don't care about
...
//calc_predictedDose should take clocktime as an arguement.  That way it can work with the actual clocktime or a hypothetical clocktime    
