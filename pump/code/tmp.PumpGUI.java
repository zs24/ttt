...
//            stopButton.setEnabled(false);
   ...
   private JPanel dosePanel () {
      ...
      textBag.gridx=1;
      textBag.gridy=row;
      parmPanel.add(this.makeStart(),textBag);
      ...
      final JTextField predDoseText = this.makeDosePredicted();

      textBag.gridx=1;
      textBag.gridy=row;
      parmPanel.add(predDoseText,textBag);
      ...
      calcButton.addActionListener(new ActionListener() {
         public void actionPerformed (ActionEvent ae) {
            predDoseText.setVisible(false);
            predDoseText.setVisible(true);
      ...
      PumpTextField tmp = new PumpTextField(8) {
         ...
         public boolean test (boolean dialogs) {
            ...
            if ( parms.get("user/time0") == null || ((String)parms.get("user/time0")).length() < 8 ) {
               // time0 required to validate clocktime.
            ...
            // following segment compares the time the pump started turning
            // to the time the user says the patient was infused.  Don't need
            // to do this check if there was no computer controlled infusion.
            // It may be that this is a dose-only run.  infused is a reference
            // to the infused flag in the parent PumpGUI object which is set
            // when someone presses the infuse button. 
            if ( infused ) {
               if ( value.before(injtime) ) {
                  String msg = "Inj Time preceeds the time the pump started turning";
                  this.setBackground(Color.RED);
                  this.setForeground(Color.WHITE);
                  this.setToolTipText(msg);
                  if ( dialogs ) {
            ...
            if ( infused ) {
               Double diff = (value.getTime() - injtime.getTime())/(1000.0);
               if ( diff > 10 ) {
                  String msg = "There is more than a 10 sec lag between \n";
                  msg += "the Inj Time and the time the pump started turning.\n";
                  this.setBackground(Color.YELLOW);
                  this.setForeground(Color.BLACK);
                  this.setToolTipText(msg);
                  if ( dialogs ) {
...
//          }
      ...
      tmp.setExternalData(parms,"user/clocktime",PumpParameters.Types.STRING);
   ...
   private JTextField makeResTime () {
      ...
      PumpTextField tmp = new PumpTextField(8) {
         ...
         public boolean test (boolean dialogs) {
            ...
            if ( parms.get("user/clocktime") == null || ((String)parms.get("user/clocktime")).length() < 8) {
               // clocktime required to validate restime
               // if clock time is missing, assume whatever user entered is bad
            ...
            Timestamp value = Timestamp.valueOf(datetime);
            Timestamp now = new Timestamp(System.currentTimeMillis());
            Timestamp clocktime = (Timestamp)parms.get("user/clocktime",PumpParameters.Types.TIME);            
            this.setText(time);
            
            if ( ! value.after(clocktime) ) {
...
// fall somewhere between these min and max values.  If not throw an error.
   public void pumpInfo (Map map) {
      ...
      if ( status != '>' ) { rate = 0; }
      if ( status == '?' ) { rate = -99; }
      this.ratePanel.addData(new Point2D.Double(runtime,rate));
      this.volumePanel.addData(new Point2D.Double(runtime,vol));
...
//       }
   ...
   public void stopPump () {      
      try { 
         ...
         String msg="";
         msg += "Received error in response to pump stop command.\n";
         msg += "\n";
